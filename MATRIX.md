...
- `vectors/` contains **664** runnable execution vectors in the `test_vectors` schema.
- The published suite has **no** `runnable: false` vectors.
- Composition: **627** L1 state-transition vectors (`input.tx` present) + **15** L0 negative wire-decoding vectors + **15** L2 block vectors (`input.kind="block"`) + **7** L2 chain-import vectors (`input.kind="chain"`).
...
627/664 vectors are L1 state transitions. L0 wire-format coverage in published
vectors is currently negative-only (15 malformed `wire_hex` vectors). The
priority gaps are:
- L2: basic executable block processing tests (22 vectors: 15 `block` + 7 `chain`)
...
| block | `execution/transactions/block/` | 15 | L2 block processing (multi-tx, atomic rejection) |
| blockchain | `execution/transactions/blockchain/` | 7 | L2 chain import (rewards + invalid tips) |
...
- Total published vectors: **664**
- L1 state-transition vectors: **627** (`input.tx` present)
- L0 negative wire-decoding vectors: **15** (`wire_format_negative`)
- L2 block vectors: **15** (`input.kind="block"`)
- L2 chain-import vectors: **7** (`input.kind="chain"`)
