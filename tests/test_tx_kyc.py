...
def test_bootstrap_committee_insufficient_fee(state_test_group) -> None:
    """bootstrap_committee with balance below fee must fail: INSUFFICIENT_FEE (pre-check)."""
    state = ChainState(network_chain_id=CHAIN_ID_DEVNET)
    sender = MINER  # Devnet bootstrap address holder (seed 1)
    state.accounts[sender] = AccountState(address=sender, balance=99_999, nonce=5)
    members = [
        {"public_key": _addr(10 + i), "name": f"member_{i}", "role": 0}
        for i in range(MIN_COMMITTEE_MEMBERS)
    ]
    ...
    state_test_group(
        "transactions/kyc/bootstrap_committee.json",
        "bootstrap_committee_insufficient_fee",
        state,
        tx,
    )
...
def test_register_committee_insufficient_fee(state_test_group) -> None:
    """register_committee with balance below fee must fail: INSUFFICIENT_FEE (pre-check)."""
    state = _base_state()
    sender = ALICE
...
def test_register_committee_nonce_too_low(state_test_group) -> None:
    state = _base_state()
    sender = ALICE
    state.committees[_hash(50)] = _global_committee()
    ...
    tx = _mk_kyc_tx(sender, nonce=4, tx_type=TransactionType.REGISTER_COMMITTEE, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/register_committee.json",
        "register_committee_nonce_too_low",
        state,
        tx,
    )


def test_register_committee_nonce_too_high_strict(state_test_group) -> None:
    state = _base_state()
    sender = ALICE
    state.committees[_hash(50)] = _global_committee()
    ...
    tx = _mk_kyc_tx(sender, nonce=6, tx_type=TransactionType.REGISTER_COMMITTEE, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/register_committee.json",
        "register_committee_nonce_too_high_strict",
        state,
        tx,
    )
...
def test_update_committee_insufficient_fee(state_test_group) -> None:
    """update_committee with balance below fee must fail: INSUFFICIENT_FEE (pre-check)."""
    state = _base_state()
    sender = ALICE
    state.accounts[sender].balance = 99_999
    state.committees[_hash(50)] = _global_committee()
    ...
    state_test_group(
        "transactions/kyc/update_committee.json",
        "update_committee_insufficient_fee",
        state,
        tx,
    )
...
def test_update_committee_nonce_too_low(state_test_group) -> None:
    ...
    tx = _mk_kyc_tx(sender, nonce=4, tx_type=TransactionType.UPDATE_COMMITTEE, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/update_committee.json",
        "update_committee_nonce_too_low",
        state,
        tx,
    )


def test_update_committee_nonce_too_high_strict(state_test_group) -> None:
    state = _base_state()
    sender = ALICE
    state.committees[_hash(50)] = _global_committee()
    ...
    tx = _mk_kyc_tx(sender, nonce=6, tx_type=TransactionType.UPDATE_COMMITTEE, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/update_committee.json",
        "update_committee_nonce_too_high_strict",
        state,
        tx,
    )
...
def test_set_kyc_insufficient_fee(state_test_group) -> None:
    """set_kyc with balance below fee must fail: INSUFFICIENT_FEE (pre-check)."""
    state = _base_state()
    sender = ALICE
    state.accounts[sender].balance = 99_999
    target = EVE
    ...
    payload = {
        ...
        "approvals": [
            _sign_approval(
                CAROL,
                _build_set_kyc_msg(
                    _hash(50), target, VALID_KYC_LEVELS[1], _hash(40), _CURRENT_TIME, _CURRENT_TIME,
                ),
                _CURRENT_TIME,
            ),
            _sign_approval(
...
def test_revoke_kyc_insufficient_fee(state_test_group) -> None:
    """revoke_kyc with balance below fee must fail: INSUFFICIENT_FEE (pre-check)."""
    state = _base_state()
    sender = ALICE
    state.accounts[sender].balance = 99_999
    target = EVE
...
def test_revoke_kyc_nonce_too_low(state_test_group) -> None:
    state = _base_state()
    sender = ALICE
    target = EVE
    state.accounts[target] = AccountState(address=target, balance=0, nonce=0)
    ...
    tx = _mk_kyc_tx(sender, nonce=4, tx_type=TransactionType.REVOKE_KYC, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/revoke_kyc.json",
        "revoke_kyc_nonce_too_low",
        state,
        tx,
    )


def test_revoke_kyc_nonce_too_high_strict(state_test_group) -> None:
    state = _base_state()
    sender = ALICE
    target = EVE
    state.accounts[target] = AccountState(address=target, balance=0, nonce=0)
    ...
    tx = _mk_kyc_tx(sender, nonce=6, tx_type=TransactionType.REVOKE_KYC, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/revoke_kyc.json",
        "revoke_kyc_nonce_too_high_strict",
        state,
        tx,
    )
...
def test_transfer_kyc_insufficient_fee(state_test_group) -> None:
    """transfer_kyc with balance below fee must fail: INSUFFICIENT_FEE (pre-check)."""
    state = _base_state()
    sender = ALICE
    state.accounts[sender].balance = 99_999
    target = EVE
...
def test_transfer_kyc_nonce_too_low(state_test_group) -> None:
    ...
    tx = _mk_kyc_tx(sender, nonce=4, tx_type=TransactionType.TRANSFER_KYC, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/transfer_kyc.json",
        "transfer_kyc_nonce_too_low",
        state,
        tx,
    )


def test_transfer_kyc_nonce_too_high_strict(state_test_group) -> None:
    state = _base_state()
    sender = ALICE
    target = EVE
    state.accounts[target] = AccountState(address=target, balance=0, nonce=0)
    ...
    tx = _mk_kyc_tx(sender, nonce=6, tx_type=TransactionType.TRANSFER_KYC, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/transfer_kyc.json",
        "transfer_kyc_nonce_too_high_strict",
        state,
        tx,
    )
...
def test_appeal_kyc_insufficient_fee(state_test_group) -> None:
    """appeal_kyc with balance below fee must fail: INSUFFICIENT_FEE (pre-check)."""
    state = _base_state()
    target = EVE
    state.accounts[target] = AccountState(address=target, balance=99_999, nonce=5)
    state.committees[_hash(50)] = _global_committee()
    state.committees[_hash(51)] = _regional_committee()
    state.kyc_data[target] = KycData(
        level=VALID_KYC_LEVELS[1],
...
def test_appeal_kyc_nonce_too_low(state_test_group) -> None:
    ...
    tx = _mk_kyc_tx(target, nonce=4, tx_type=TransactionType.APPEAL_KYC, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/appeal_kyc.json",
        "appeal_kyc_nonce_too_low",
        state,
        tx,
    )


def test_appeal_kyc_nonce_too_high_strict(state_test_group) -> None:
    state = _base_state()
    target = EVE
    state.accounts[target] = AccountState(address=target, balance=10_000_000, nonce=5)
    state.committees[_hash(50)] = _global_committee()
    ...
    tx = _mk_kyc_tx(target, nonce=6, tx_type=TransactionType.APPEAL_KYC, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/appeal_kyc.json",
        "appeal_kyc_nonce_too_high_strict",
        state,
        tx,
    )
...
def test_emergency_suspend_insufficient_fee(state_test_group) -> None:
    """emergency_suspend with balance below fee must fail: INSUFFICIENT_FEE (pre-check)."""
    state = _base_state()
    sender = ALICE
    state.accounts[sender].balance = 99_999
    target = EVE
...
def test_emergency_suspend_nonce_too_low(state_test_group) -> None:
    state = _base_state()
    sender = ALICE
    target = EVE
    state.accounts[target] = AccountState(address=target, balance=0, nonce=0)
    ...
    tx = _mk_kyc_tx(sender, nonce=4, tx_type=TransactionType.EMERGENCY_SUSPEND, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/emergency_suspend.json",
        "emergency_suspend_nonce_too_low",
        state,
        tx,
    )


def test_emergency_suspend_nonce_too_high_strict(state_test_group) -> None:
    state = _base_state()
    sender = ALICE
    target = EVE
    state.accounts[target] = AccountState(address=target, balance=0, nonce=0)
    ...
    tx = _mk_kyc_tx(sender, nonce=6, tx_type=TransactionType.EMERGENCY_SUSPEND, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/emergency_suspend.json",
        "emergency_suspend_nonce_too_high_strict",
        state,
        tx,
    )
...
def test_renew_kyc_insufficient_fee(state_test_group) -> None:
    """renew_kyc with balance below fee must fail: INSUFFICIENT_FEE (pre-check)."""
    state = _base_state()
    sender = ALICE
    state.accounts[sender].balance = 99_999
    target = EVE
...
def test_renew_kyc_nonce_too_low(state_test_group) -> None:
    state = _base_state()
    sender = ALICE
    target = EVE
    state.accounts[target] = AccountState(address=target, balance=0, nonce=0)
    ...
    tx = _mk_kyc_tx(sender, nonce=4, tx_type=TransactionType.RENEW_KYC, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/renew_kyc.json",
        "renew_kyc_nonce_too_low",
        state,
        tx,
    )


def test_renew_kyc_nonce_too_high_strict(state_test_group) -> None:
    state = _base_state()
    sender = ALICE
    target = EVE
    state.accounts[target] = AccountState(address=target, balance=0, nonce=0)
    ...
    tx = _mk_kyc_tx(sender, nonce=6, tx_type=TransactionType.RENEW_KYC, payload=payload, fee=100_000)
    state_test_group(
        "transactions/kyc/renew_kyc.json",
        "renew_kyc_nonce_too_high_strict",
        state,
        tx,
    )
